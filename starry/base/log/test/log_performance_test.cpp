...
protected:
    void SetUp() override {
        // 初始化异步日志系统
        asyncLog_ = std::make_unique<AsyncLogging>("test_log", "/tmp", 64 * 1024 * 1024);
        asyncLog_->start();
...
TEST_F(LogPerformanceTest, ThroughputTest) {
    ...
    const std::string message = "This is a test log message for throughput measurement";
...
TEST_F(LogPerformanceTest, LatencyTest) {
    ...
    const std::string message = "This is a test log message for latency measurement";
